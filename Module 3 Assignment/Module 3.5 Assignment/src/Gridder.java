...
public class Gridder extends javax.swing.JFrame 
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        int max = 100;
        for(int i = 0; i < gridCount; i++) {    //selects random x and y value for square
            int xValue = (int) (Math.random() * (max));
            int yValue = (int) (Math.random() * (max));
            grid[xValue][yValue] = 1;
        }
        draw();
    }//GEN-LAST:event_jButton1ActionPerformed
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        int column = (int) (Math.random() * (gridCount));
        for(int row = 0; row < gridCount; row++) {    //iterates over every row in int column
            grid[column][row] = 1;
        }
        draw();
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        clearGrid();
        for(int col = 0; col < gridCount; col++) {    //iterates over every column
            for(int row = 0; row < gridCount / 2; row++) {    //iterates over first 50 rows
                grid[col][row] = 1;
            }
        }
        draw();
    ...
    private void jButton5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        ...
        for(int col = 0; col < gridCount; col++) {    //prints NE to SW line
            for(int row = 0; row < gridCount; row++) {
                if(gridCount - col - 1 == row) {
                    grid[col][row] = 1;
                }
            }
        }
    ...
    private void jButton7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        ...
        for(int row = 0; row < gridCount; row++) {
            for(int col = 0; col < gridCount; col++) {
                ...
                //defining offsets to check directions
                int[] above = {0, -1};
                int[] below = {0, 1};
                int[] left = {-1, 0};
                int[] right = {1, 0};
                ...
                if(col == 0 & row == 0) {    //runs if square is the top left corner
                    int[][] directions = {below, right, belowRight};
                    whiteTouches += checkWhiteTouches(directions, col, row);
                }
                if(col == gridCount - 1 & row == 0) {    //runs if square is the top right corner
                    int[][] directions = {below, left, belowLeft};
                    whiteTouches += checkWhiteTouches(directions, col, row);
                }
                if(col == 0 & row == gridCount - 1) {    //runs if square is the bottom left corner
                    int[][] directions = {above, right, aboveRight};
                    whiteTouches += checkWhiteTouches(directions, col, row);
                }
                if(col == gridCount - 1 & row == gridCount - 1) {    //runs if square is the bottom right corner
                    int[][] directions = {above, left, aboveLeft};
                    whiteTouches += checkWhiteTouches(directions, col, row);
                }
                if(col != 0 & col != gridCount - 1 & row == 0) {    //runs if square is the upper border
                    int[][] directions = {below, left, right, belowLeft, belowRight};
                    whiteTouches += checkWhiteTouches(directions, col, row);
                }
                if(col != 0 & col != gridCount - 1 & row == gridCount - 1) {    //runs if square is the lower border
                    int[][] directions = {above, left, right, aboveLeft, aboveRight};
                    whiteTouches += checkWhiteTouches(directions, col, row);
                }
                if(col == 0 & row != 0 & row != gridCount - 1) {    //runs if square is the left border
                    int[][] directions = {above, below, right, aboveRight, belowRight};
                    whiteTouches += checkWhiteTouches(directions, col, row);
                }
                if(col == gridCount - 1 & row != 0 & row != gridCount - 1) {    //runs if square is the right border
                    int[][] directions = {above, below, left, aboveLeft, belowLeft};
                    whiteTouches += checkWhiteTouches(directions, col, row);
                }
                if(col != 0 & col != gridCount - 1 & row != 0 & row != gridCount - 1){    //runs if square is contained within all borders
                    int[][] directions = {above, below, left, right, aboveLeft, aboveRight, belowLeft, belowRight};
                    whiteTouches += checkWhiteTouches(directions, col, row);
                }
                if (whiteTouches >= 2) {
                    temp[col][row] = 1;
    ...
    private int checkWhiteTouches(int[][] directions, int col, int row) {    //helper method for jButton7ActionPerformed
        int whiteTouches = 0;
    ...
    private void jButton8ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        ...
        int leftwardOffset = 1;    //amount of squares that the image is "scrolled"
        for(int col = leftwardOffset; col < gridCount; col++) {    //shifts image left by leftwardOffset
            for(int row = 0; row < gridCount; row++) {
                temp[col - leftwardOffset][row] = temp[col][row];
            }
        }
        for(int col = 0; col < leftwardOffset; col++) {    //moves leftmost columns to the right side
            for(int row = 0; row < gridCount; row++) {
                temp[gridCount - leftwardOffset + col][row] = grid[col][row];
            }
        }
        grid = temp;
        draw();
    ...
    private void jButton9ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        ...
        int numberOffset = 1;    //ensures that loop does not go out of bounds
        for(int col = 0; col < gridCount; col++) {
            for(int row = 0; row < gridCount; row++) {
                temp[gridCount - row - numberOffset][col] = grid[col][row];
            }
        }
        grid = temp;
        draw();
